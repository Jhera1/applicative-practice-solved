...
const getPlanetsWithMassValue = (data, number) => {
  let result = []
...
// console.log(getPlanetsWithMassValue(data, 7))


function getAsteroidsDiscoveredAfterYear(data, year) {
  // Your code goes here...
  return data.asteroids.filter(a => a.discoveryYear > year).map(a => a.name)
  
}
// console.log(getAsteroidsDiscoveredAfterYear(data, 1989))

const getPlanetsNamesWithMoons = (data) => {
  let result = []
  for (let i = 0; i < data.planets.length; i++) {
...
function findPlanetNameByMoon(data, moonName) {
  ...
  // console.log(findPlanetNameByMoon(data, 'Obéron'))



  function lowMoonsPlanets(data) {
    // Your code goes here...
  return data.planets.filter(a => !a.moons || a.moons.length < 10)
                     .map(a => a.name)
  ...
  function allPlanetsMoonsCount(data) {
    // Your code goes here...
  return data.planets.map(a => !a.moons ? 0 : a.moons.length)
                     .reduce((a, b) => a + b)
  }
  ...
  function getAveragePlanetsTemperature(data) {
   ...
   let allTemps = []
   for (let i = 0; i < data.planets.length; i++) {
    allTemps.push(data.planets[i].avgTemp)
   }
   return Number((allTemps.reduce((a, b) => a + b, 0) / allTemps.length).toFixed(2))
  }
  // console.log(getAveragePlanetsTemperature(data))

  function getPlanetsWithNoMoons(data) {
    // Your code goes here...
    return data.planets.filter(a => !a.moons)
  }
